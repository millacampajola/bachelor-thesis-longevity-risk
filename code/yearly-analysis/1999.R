...
LC_fit_1999 <- fit(LC, data = ITA, ages.fit = 0:90, years.fit = 1949:1999)
LC_boot_1999 <- bootstrap(LC_fit_1999, nBoot = 1000, type = "semiparametric")
LC_sim_boot_1999 <- simulate(LC_boot_1999, h = 25)

## ---- estrazione coorte ----

coorte <- 1934 # coorte da estrarre

# anni previsti
anni_forecast <- 2000:(2000+25-1)  # perché h = 25
eta <- 0:90
...
rates_coorte <- sapply(1:nrow(indici), function(i) {
  LC_sim_boot_1999$rates[indici[i, 2], indici[i, 1], ]
})
...
annuities_1999 <- array(NA, dim = 1000)
for (j in 1:1000){
  annuities_1999[j] <- annuity(v, R, surv_probs[j, ]) 
}

# istogramma
summary_1999 <- summary(annuities_1999)
hist(annuities_1999)
points(mean(annuities_1999), 0, col = "blue", pch = 19, cex = 1.5)
